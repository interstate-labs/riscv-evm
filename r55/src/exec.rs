...
pub fn deploy_contract(db: &mut InMemoryDB, bytecode: Bytes) -> Address {
  ...
  match result {
    ExecutionResult::Success { output: Output::Create(_value, Some(addr)), .. } => {
...
fn riscv_context(frame: &Frame) -> Option<RVEmu> {
  ...
  if matches!(interpreter.bytecode.get(0), Some(0xFF)) {
    ...
    Some(RVEmu { emu, returned_data_destiny: None })
  } else {
    None
  }
}
      ...
      Err(Exception::EnvironmentCallFromMMode) => {
        ...
        match t0 {
          ...
          2 => {
            ...
            host.sstore(interpreter.contract.target_address, U256::from(key), U256::from(value));
          }
          3 => {
            // Syscall::Call
